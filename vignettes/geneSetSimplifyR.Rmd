...
-   Optionally(!) meta data about the enrichment analysis that can be used for downstream visualization. Specifically `geneSetSimplifyR` supports visualization of
...
This can naturally be followed up by e.g. a UMAP at a different resolution — here resolution 1.6:
...
In this case we might determine that this resolution better fits our data. Let's update the default resolution:
...
It is common to find that part of an auto-generated cluster label comes from an author or database prefix in the gene-set names rather than from the underlying biology — in such cases you will want to override the label manually. For example, if you decide cluster 0 is better described as "cancer response" (because those are the most frequent informative words and they summarize the most central gene-sets), the gsList can be updated to reflect this:
...
We recommend this table to ALWAYS be added to the supplementary material of an article(!) as it is key to both transparency and reproducibility!
...
In this toy data there are only 59 significant gene sets, but that is because it is an artificial dataset only meant to illustrate the workflow. As discussed in the introduction, 1000+ significant gene sets are very common. But even though it is possible to manually consider 59 gene sets, let's use `geneSetSimplifyR` to analyze them anyway. This could be done as follows (not run here since the workflow is already covered above in the [geneSetSimplifyR workflow] section):
